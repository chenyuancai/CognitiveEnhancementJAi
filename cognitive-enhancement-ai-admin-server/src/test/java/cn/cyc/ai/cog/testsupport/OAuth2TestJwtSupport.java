package cn.cyc.ai.cog.testsupport;
...
/**
 * 集成测试用 OAuth2 RS256 JWT 签发工具。
 */
public final class OAuth2TestJwtSupport {

    private static final RSAKey RSA_KEY;

    static {
        try {
            RSA_KEY = new RSAKeyGenerator(2048).keyID("integration-test-key").generate();
        } catch (Exception ex) {
            throw new IllegalStateException("初始化测试 RSA 密钥失败", ex);
        }
    }

    private OAuth2TestJwtSupport() {
    }
    ...
    public static String oauth2BearerToken(Long userId, String username, String tenantCode, List<String> roles) {
        try {
            ...
            JWTClaimsSet claims = new JWTClaimsSet.Builder()
                    ...
                    .claim(SecurityConstants.CLAIM_TENANT_ID, 1L)
