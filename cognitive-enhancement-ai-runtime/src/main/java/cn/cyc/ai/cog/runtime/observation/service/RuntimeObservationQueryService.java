...
public class RuntimeObservationQueryService {
     ...
     * 观测统计聚合器。
     */
    private final ObservationStatsAggregator observationStatsAggregator;
    ...
    public RuntimeObservationQueryService(ExecutionRecordRepository executionRecordRepository,
                                          ...
                                          ObservationStatsAggregator observationStatsAggregator) {
        this.executionRecordRepository = executionRecordRepository;
        this.usageRecordRepository = usageRecordRepository;
        this.modelCheckRecordRepository = modelCheckRecordRepository;
        this.observationStatsAggregator = observationStatsAggregator;
    }
    ...
    public RuntimeListResult<ExecutionRecord> listExecutionRecords(String traceId,
        ...
        log.info("查询执行记录列表, traceId={}, capabilityCode={}, agentCode={}, startTime={}, endTime={}, total={}, page={}, size={}, sort={}",
                traceId, capabilityCode, agentCode, startTime, endTime, items.size(), result.page(), result.size(), sort);
        return result;
    }

    /**
     * 按 traceId 查询执行链路详情（含关联用量）。
     *
     * @param traceId 链路标识
     * @return 执行链路详情
     */
    public ExecutionRecordDetail getExecutionRecordDetail(String traceId) {
        ExecutionRecord execution = executionRecordRepository.findByTraceId(traceId)
                .orElseThrow(() -> new BusinessException("NOT_FOUND", "未找到执行记录: " + traceId));
        List<UsageRecord> usages = usageRecordRepository.listAll().stream()
                .filter(record -> Objects.equals(record.traceId(), traceId))
                .toList();
        log.info("查询执行链路详情, traceId={}, usageCount={}", traceId, usages.size());
        return new ExecutionRecordDetail(execution, usages);
    }
    ...
    public RuntimeListResult<UsageRecord> listUsageRecords(String traceId,
        ...
        log.info("查询用量记录列表, traceId={}, capabilityCode={}, agentCode={}, startTime={}, endTime={}, total={}, page={}, size={}, sort={}",
                traceId, capabilityCode, agentCode, startTime, endTime, items.size(), result.page(), result.size(), sort);
        return result;
    }
    ...
    public RuntimeListResult<ModelCheckRecord> listModelCheckRecords(String traceId,
        ...
        log.info("查询模型检查记录列表, traceId={}, providerCode={}, modelCode={}, startTime={}, endTime={}, total={}, page={}, size={}, sort={}",
                traceId, providerCode, modelCode, startTime, endTime, items.size(), result.page(), result.size(), sort);
        return result;
    }
     ...
     * @return 聚合统计结果
     */
    public ObservationStatsResult aggregateStats(Instant startTime, Instant endTime) {
        return observationStatsAggregator.aggregate(startTime, endTime);
    }
