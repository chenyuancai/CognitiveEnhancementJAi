...
class ModelConnectivityControllerTest {
    ...
    void shouldListModelStatusesWithLatestCheckSummary() throws Exception {
        ...
        mockMvc.perform(get("/api/runtime/models/statuses"))
                ...
                .andExpect(jsonPath("$.data.items[3].providerCode", is("disabled-provider")))
                .andExpect(jsonPath("$.data.items[3].modelCode", is("model.disabled")))
                .andExpect(jsonPath("$.data.items[3].status", is("DISABLED")))
                .andExpect(jsonPath("$.data.items[3].healthStatus", is("DISABLED")));
    ...
    void shouldQueryModelStatusOverviewWithFailureAggregation() throws Exception {
        ...
        mockMvc.perform(get("/api/runtime/models/overview"))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.success", is(true)))
                .andExpect(jsonPath("$.data.totalModels", is(4)))
                .andExpect(jsonPath("$.data.enabledModels", is(3)))
                .andExpect(jsonPath("$.data.disabledModels", is(1)))
                ...
                .andExpect(jsonPath("$.data.uncheckedModels", is(2)))
